...
const HeroFooter = () => {
  ...
  return (
    <div className="w-[calc(100%-20px)] lg:w-full absolute left-1/2 -translate-x-1/2 bottom-6 flex items-center justify-between text-[9px] md:text-sm lg:text-base select-none tracking-wider ">
      {/* Email */}
      <button
        className="relative z-10 group text-text-secondary-alt bg-bg-primary"
        onClick={handleCopy}
      ...
      >
        <span className="flex items-center gap-1 cursor-pointer ">
          ...
          <div
            className="group-hover:text-text-primary hover-transition font-suisse-light"
          >
            <Zoop text={email} />
          </div>
        </span>
        ...
        <motion.span
          className={`absolute left-1/2 -translate-x-1/2 -top-8 md:-top-10 font-caveat text-xs md:text-base lg:text-xl whitespace-nowrap ${copied && "text-blue"} 
          `}
      ...
      {/* Location */}
      <span className="group flex items-center gap-1 text-text-secondary-alt">
        <MapPin className="w-3 h-3 md:w-4 md:h-4 group-hover:text-text-primary hover-transition" />
        <div className="group-hover:text-text-primary hover-transition font-suisse-light">
          <Zoop text={location} />
        </div>
      </span>
    </div>
